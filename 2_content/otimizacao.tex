...
\newacronym{mhe}{MHE}{\textit{Moving Horizon Estimation}}
...
\label{subsec:metodo_descida_mais_ingrime}
...
\lstinputlisting[	
	caption={[Busca por descidas mais íngremes]
			Busca por descidas mais íngremes \\
		    Fonte: Autor, adaptado de \citeonline{Haugen2018}},
...
$x_0 = 12$, e as derivadas consecutivas a partir desde valor converger para o
mínimo global; já na \cref{fig:steepest_descent_local_minimum} estimasse $x_0 = 10$
resultando em um $f_{min}$ completamente diferente, convergendo para um mínimo local.
Em ambas as figuras a linha verde indica o valor de $x_0$ e a linha vermelha indica
o $f_{min}$ qaundo o critério de parada é atingido.
...
do método de busca de descidas mais íngrimes, porém outros métodos podem levar esse tipo
...
A comparação o sistema real e o virtual é feita atravéz do acumulo da diferença das saídas dos
...
\begin{itemize}
	...
	\item $\mathrm{f}$ é um vetor de $n$ funções lineares ou não-lineares de $x_k$
		\begin{equation}
			...
			\end{bmatrix}
		\end{equation}
		O pontos representam possíveis argumentos adicionais de $f$ como a variável de
		controle ($u_k$), os disturbios do processo ($d_k$), e os parâmetros ($p$).
...
Onde: 
\begin{itemize}
	\item $\mathrm{w}_i$ é o vetor de disturbio do processo.
		\begin{equation}
			\mathrm{w}_i =
			\begin{bmatrix}
				w(1)_{i} \\
	...
	\item $\mathrm{Q}^{-1}$ é a matriz de ponderação de $\mathrm{w}_i$
		...
		da matriz de covariância do disturbio do processo.
...
\lstinputlisting[	
	caption={[Exemplo do método Moving Horizon Estimation]
	Exemplo do método Moving Horizon Estimation \\
	Fonte: Autor, adaptado de \citeonline{Haugen2018}},
