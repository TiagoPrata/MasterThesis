...
encontramos os valores descritos nas equações descritas em \ref{equ_grid_search_vect_bounds_output}.
...
As figuras \ref{fig_grid_search_vectorial_nobounds} e \ref{fig_grid_search_vectorial_withbounds}
mostram a função custo (outra forma de chamarmos a função objetivo) sem restrição e com a
restrição descrita pela inequação \ref{equ_min_restr_desigualdade}, respectivamente.
...
A figura \ref{fig_steepest_decent_slope} mostra graficamente a influência da derivada
...
\begin{figure}
	\begin{center}
		...
		\label{fig_steepest_decent_slope}
