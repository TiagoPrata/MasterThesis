...
% ___________________________________________ Paragraph _______________________________________________
% _____________________________________________________________________________________________________
\paragraph*{\textbf{Modelos experimentais NLARX}}
\label{par:modelos_experimentais_nlarx}

Os modelos \acrshort{nlarx} foram criados a partir de combinações de diferentes de
$n_a$ (matriz onde $n_a(i,j)$ especifica o número de regressores da j-ésima saída usados para predizer
a i-ésima saída),
$n_b$ (matriz onde $n_b(i,j)$ especifica o número de regressores da j-ésima entrada usados para predizer
a i-ésima saída) e
$n_k$ (matriz onde $n_k(i,j)$ especifica o atraso na j-ésima entrada usados para predizer
a i-ésima saída).
As combinações utilizadas podem ser vistas na
\cref{tab:tclabsp-models-nlarx}.

\begin{table}[h]
	\centering
	\caption{Modelos experimentais - NLARX}
	\label{tab:tclabsp-models-nlarx}
	\begin{tabular}{c|c} \toprule
		{Parâmetro}		&	{Valores utilizados nas combinações}			\\ \midrule
		$n_a$			&
							$ \begin{bmatrix}	
...
A partir de todos os modelos possíveis gerados a partir das combinações dos parâmetros da \cref{tab:tclabsp-models-nlarx},
selecionaram-se 10\% dos modelos com menor valor segundo o critério de \acrshort{aic} e a partir destes foi então
selecionado o modelo com maior proximidade do conjunto de dados de validação, uma vez que a análise residual
apresentou valores muito próximos para todos os modelos testados.
O gráfico mostrando a distribuição dos modelos gerados em comparação ao critério de \acrshort{aic} pode ser
visto na \cref{fig:tclabsp-models-nlarx} e o modelo \acrshort{nlarx} escolhido, seu gráfico comparativo aos dados validação
e sua análise residual podem ser vistos na \cref{tab:tclabsp-model-nlarx}, na \cref{fig:tclabsp-models-nlarx-compare} e na
\cref{fig:tclabsp-models-nlarx-resid}, respectivamente.

\begin{figure}[h]
	\caption{Modelos experimentais NLARX}
	\begin{center}
		\includegraphics[width=1.00\textwidth]{./5_images/tclabsp-models-NLARX.png} 
		\label{fig:tclabsp-models-nlarx}
	\end{center}
	\centering
	\makebox[\width]{Fonte: Autor} 
\end{figure}
...
\begin{table}[h]
	\centering
	\caption{Melhor modelo experimental - NLARX}
	\label{tab:tclabsp-model-nlarx}
	\begin{tabular}{c|c} \toprule
		...
		$Sensor_1$			&
								\begin{tabular}[c]{@{}l@{}}
									\textbf{Regressores não-lineares}		\\
									- $Sensor_1 (t-1)$				\\
									- $Aquecedor_1 (t)$				\\
									- $Aquecedor_1 (t-1)$				\\
									- $Aquecedor_2 (t-2)$				\\
									- $Aquecedor_2 (t-3)$				\\
									\textbf{Não-linearidades}		\\
									- Wavenet com 2 unidades
								\end{tabular}
						\\ \midrule
		$Sensor_2$			&
								\begin{tabular}[c]{@{}l@{}}
...
\begin{figure}[h]
	\caption{Modelo: NLARX - Teste de validação}
	\begin{center}
		\includegraphics[width=1.00\textwidth]{./5_images/tclabsp-models-NLARX-compare.png} 
		\label{fig:tclabsp-models-nlarx-compare}
	\end{center}
	\centering
	\makebox[\width]{Fonte: Autor} 
\end{figure}
...
\begin{figure}[h]
	\caption{Modelo: NLARX - Análise de resíduos}
	\begin{center}
		\includegraphics[width=1.00\textwidth]{./5_images/tclabsp-models-NLARX-resid.png} 
		\label{fig:tclabsp-models-nlarx-resid}
	\end{center}
	\centering
	\makebox[\width]{Fonte: Autor} 
\end{figure}
